...
Modified for integration in a balibase benchmark (2019).
"""
...
def myGetResult(jobId, outfilename=None, fmt=None):
    printDebugMessage(u'getResult', u'Begin', 1)
    printDebugMessage(u'getResult', u'jobId: ' + jobId, 1)
    if outputLevel > 1:
        print("Getting results for job %s" % jobId)
    ...
    for resultType in (r for r in resultTypes if (not fmt) or unicode(r[u'fileSuffix']) == unicode(fmt)):
        # Derive the filename for the result
        if outfilename:
...
[Optional]
  --guidetreeout        Output guide evolutionaryInference.
  --dismatout           Output distance matrix. This is only calculated if the mBed-
                        like clustering guide evolutionaryInference is set to false.
  ...
  --mbed                This option uses a sample of the input sequences and then
                        ...
                        enabling much more rapid generation of the guide evolutionaryInference,
                        especially when the number of sequences is large.
  --mbediteration       Use mBed-like clustering during subsequent iterations.
  --iterations          Number of (combined guide-evolutionaryInference/HMM) iterations.
  --gtiterations        Having set the number of combined iterations, this parameter
                        can be changed to limit the number of guide evolutionaryInference iterations
...
def runClustalO(mail, seqs, outfilename=None, fmt='fasta'):
    params = {}
    params[u'email'] = mail
    params['outfmt'] = 'fa' if fmt == 'fasta' else fmt  # probably there will be some pb if not fasta
    if os.path.isfile(seqs):
        params[u'sequence'] = readFile(seqs)
    else:
        params[u'sequence'] = seqs
    jobId = myServiceRun(mail, params)
    if outputLevel > 0:
        print("JobId: " + jobId, file=sys.stderr)
    else:
        print(jobId)
    time.sleep(pollFreq)
    myGetResult(jobId, outfilename, fmt=fmt)
